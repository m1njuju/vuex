...
export default new Vuex.Store({
  ...
  getters: {
    doneTodosCountGetter: function(state) {
      // 동일한 형식으로 사용할 수 있다
      // filter( function(배열의 요소를 담는 변수) { return 참일 때 요소 값을 배열에 추가})
      return state.todos.filter(todo => todo.done).length;
    },
    todosReverse: function(state) {
      return state.todos.reverse();
    },
    // todos의 id값을 통해 todo객체 반환
  ...
  actions: {
    ...
    addcount(context) {
      //mutations의 메서드를 들고와 상태변화(state)를 추적할 수 있다
    ...
    ntimer: function(context, time) {
      setTimeout(() => {
        // Mutation의 메소드를 commit할 때 인자값을 전달하고 싶다면
        // , 를 통해 값을 같이 보내준다
